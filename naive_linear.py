...
result = mdl.predict(Xts)

np.savetxt('naive_linear_merge_result.txt', result, fmt="%d", delimiter=',')
print (mdl.score(Xts, yts))
# 0.845 best score on split data with a=0.00001, max_iter=1000, l1_ratio = 0.3
# .847 when trained on all data -> so not as good as merging with others on val
