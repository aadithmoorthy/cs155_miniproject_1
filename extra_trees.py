...
print ('loaded data')
mdl = ExtraTreesClassifier(n_estimators=1000, verbose=1, max_depth=1000, n_jobs=4)
print (mdl)
mdl.fit(Xtr, ytr)

print (mdl.score(Xtr, ytr))
#print (mdl.score(Xts, yts))
# best split with n_estimators=1000, maxdepth=500, got .852
...
np.savetxt('et_result.txt', result, fmt="%d", delimiter=',', header='Id,Prediction', comments='')
'''
